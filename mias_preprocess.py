...
for i,row in normal.iterrows():
    ...
    if(int(row['reference_number'][-3:])%2==0):
        ...
        matlabimg.imsave('dataset_png/'+ row['reference_number']+'.png',images[j],vmin=0,vmax=255,cmap='gray')
    j+=1
    ...
    def generate_patches(input_image):
        ...
        for counter, i in enumerate(patches):
            if np.any(i):
                matlabimg.imsave('dataset_crops_new/'+str(global_counter)+'.jpg',i,cmap='gray')
                global_counter+=1
